...
dev.new()
png(file.path("../report/img",paste("model_FSC", "ALL", ".png", sep="_")), width = 500, height = 1600)
...
dev.new()
png(file.path("../report/img",paste("model_SSC", "ALL", ".png", sep="_")), width = 500, height = 1600)
gridExtra::grid.arrange(grobs = all_est_plots , ncol = 1, nrow = length(all_est_plots))
dev.off()
...
dev.new()
png(file.path("../report/img",paste("model_PC5.5", "ALL", ".png", sep="_")), width = 500, height = 1600)
gridExtra::grid.arrange(grobs = all_est_plots , ncol = 1, nrow = length(all_est_plots))
dev.off()
...
event_crit <- filter(crit_pts_all_para, der == "First_der", parameter == "events") %>% 
  filter(flask != "H" & flask != "I")
FSC_crit <- filter(crit_pts_all_para, der == "Estimation", parameter == "FSC") %>% 
  filter(flask != "A")
SSC_crit <- filter(crit_pts_all_para, der == "Estimation", parameter == "SSC") %>% 
  filter(flask != "A")
PC5.5_crit <- filter(crit_pts_all_para, der == "Estimation", parameter == "PC5.5") %>% 
  filter(flask != "A")


anova_events <- aov(Critical~light_regime, data = event_crit)
anova_FSC <- aov(Critical~light_regime, data = FSC_crit)
anova_SSC <- aov(Critical~light_regime, data = SSC_crit)
anova_PC5.5 <- aov(Critical~light_regime, data = PC5.5_crit)

anova_stats <- rbind(summary(anova_events)[[1]] %>% 
        mutate(Parameter = "Events"), 
      summary(anova_FSC)[[1]] %>% 
        mutate(Parameter = "FSC"),
      summary(anova_SSC)[[1]] %>% 
        mutate(Parameter = "SSC"),
      summary(anova_PC5.5)[[1]] %>% 
        mutate(Parameter = "PC5.5"))
# export as latex table
stargazer(anova_stats, summary = FALSE)
 summary(anova_FSC)
 
 
 library(rstatix)
pairwise_FSC <- FSC_crit %>% pairwise_t_test(Critical ~ light_regime, p.adjust.method = "bonferroni") %>%
  mutate(Parameter = "FSC")
pairwise_SSC <- SSC_crit %>% pairwise_t_test(Critical ~ light_regime, p.adjust.method = "bonferroni") %>%
  mutate(Parameter = "SSC")

pairwise_stats <- rbind(pairwise_FSC, pairwise_SSC) %>%
  select(Parameter, group1, group2, p, p.signif, p.adj, p.adj.signif)
stargazer(pairwise_stats, summary = FALSE)
 

# Do another ANOVA but this time between parameters instead 

crit_pts_all_para

crit_1608 <- filter(crit_pts_all_para, der == "Estimation", light_regime == 1608) %>% 
  filter(flask != "A") %>% 
  filter(parameter != "events")

crit_1212 <- filter(crit_pts_all_para, der == "Estimation", light_regime == 1212) %>% 
  filter(flask != "A") %>% 
  filter(parameter != "events")

crit_0816 <- filter(crit_pts_all_para, der == "Estimation", light_regime == "0816") %>% 
  filter(flask != "A") %>% 
  filter(parameter != "events")

anova_1608 <- aov(Critical~parameter, data = crit_1608)
anova_1212 <- aov(Critical~parameter, data = crit_1212)
anova_0816 <- aov(Critical~parameter, data = crit_0816)

summary(anova_1608)[[1]]

anova_stats <- rbind(summary(anova_1608)[[1]] %>% 
        mutate(Light_Regime = "1608"), 
      summary(anova_1212)[[1]] %>% 
        mutate(Light_Regime = "1212"),
      summary(anova_0816)[[1]] %>% 
        mutate(Light_Regime = "0816"))
# export as latex table
stargazer(anova_stats, summary = FALSE)
 summary(anova_FSC)
...
get_table("/Users/davidmah/Desktop/Fightoplankton issues.xlsx")


table_data<- read_excel("/Users/davidmah/Desktop/sampler cost.xlsx")
stargazer(table_data, summary = FALSE)
