...
This adds all the custom functions I use throughout the script
```{r}
...
This chunk plots the preservation of samples by plotting Auto against manual samples for %change in parameter against time in the storage unit. In fact, it seems like the manual sample is more influenced by sampling error than the degredation of the samples in the storage unit are affected by time. 
```{r}
...
d_ss_mean_AM_A_ratio <-data.frame(c(d_ss_mean_AM_A[1:12],d_ss_mean_AM_A[-c(1:12)]/d_ss_mean_AM_M[-c(1:12)]))
...
# 
# FLASK <- filter(d_ss_mean, flask == "B")
...
# Tried to create my own axis transformation. Due to the nature of autoplot, it did not work :( 
# hr2day_trans <- trans_new(name = "h2d", 
#                                     transform = function(x) x/24,
#                                     inverse = function(x) x*24)

# This line gets the y values for the 1st der 
# range(autoplot(fit2, der = 1)[["layers"]][[2]][["data"]][["p"]])
...
all_der_plots <- within(all_der_plots, rm(A, H, I)) 
...
# Make the whole thing a loop? Dont waste your time lmao 
# for (col in colnames(d_ss_mean[13:16])){
#   what <- col
# }
